...
Three of those weights were hand-set magic numbers — the personalised
ones, "what predicts a fact's value *before* the user has reacted to
it". This module makes them learned from the user's own resonance
feedback: every closed session contributes one regularised SGD step
toward predicting realised value from (freshness, access, graph)
measured before that resonance landed.
...
training data is identical to the previous hand-tuned formula.
"""
...
class AdaptiveWeights:
    """The three pre-resonance weights — learned from resonance feedback."""
    ...
    # Hand-set prior: the weights at zero data. Matches the previous formula.
    PRIOR: ClassVar[tuple[float, float, float]] = (0.35, 0.20, 0.10)
    # Pre-resonance budget — the three weights sum to BUDGET. Resonance
    ...
    def predict(self, freshness: float, access: float, graph: float) -> float:
        return (
    ...
    def update(
        ...
        ``target`` is the realised value of the fact for the user — for the
        first-resonance training event, ``(R + 1) / 2`` of the session's R.
        """
        err = target - self.predict(freshness, access, graph)
        ...
        total = self.w_freshness + self.w_access + self.w_graph
        if total > 0.0:
            ...
            self.w_freshness, self.w_access, self.w_graph = self.PRIOR
        self.train_count += 1
