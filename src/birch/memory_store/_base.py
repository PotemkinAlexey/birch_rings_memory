...
class MemoryStore(
    ...
    def __init__(
        ...
        # Encoding-salience (declarative pin) telemetry. pins_created /
        # pins_resonated are DERIVED in stats from persisted per-fact flags
        # (FactPassport.was_pinned / pin_resonated) so the channel's verdict
        # accrues across restarts. Only eviction — an event, not fact state —
        # stays a process-local counter.
